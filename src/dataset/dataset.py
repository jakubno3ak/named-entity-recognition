...
def create_dataset(
    settings: Settings, tokenizer: AutoTokenizer
) -> tuple[Dataset, DatasetMetadata]:
    csv_raw_data_full_path: str = unzip_data(
        settings.zip_data_full_path, settings.csv_data_dir_path
    )
    json_datast_full_path: str = extract_data_to_json(
        data_read_path=csv_raw_data_full_path,
        data_write_path=settings.json_data_dir_path,
    )
    ...
    metadata: DatasetMetadata = DatasetMetadata(
        label_names=label_names, id_to_label=id_to_label, label_to_id=label_to_id
    )
